...
|------|--------|-------------|
| `webhook` | POST | Called by Payhawk webhook on `expense.approved`. Generates a PDF and attaches it to the expense. Body: `{"payload": {"expenseId": "123"}}` |
| `generate` | POST | Manual trigger. Same behavior as webhook. Body: `{"payload": {"expenseId": "123"}}` |
